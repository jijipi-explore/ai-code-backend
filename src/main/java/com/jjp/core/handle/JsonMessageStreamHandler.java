package com.jjp.core.handle;

import cn.hutool.core.io.FileUtil;
import cn.hutool.core.util.StrUtil;
import cn.hutool.json.JSONObject;
import cn.hutool.json.JSONUtil;
import com.jjp.ai.model.message.*;
import com.jjp.ai.tools.BaseTool;
import com.jjp.ai.tools.ToolManager;
 ...
 * @Description: 处理 VUE_PROJECT 类型的复杂流式响应，包含工具调用信息
 * @Version: 1.0
 */
...
public class JsonMessageStreamHandler {
     ...
     * 该方法用于处理接收到的JSON数据块，根据不同的消息类型进行相应的处理
     *
     * @param chunk 接收到的JSON数据块
     * @param chatHistoryStringBuilder 用于构建聊天历史记录的字符串构建器
     * @param seenToolIds 已处理过的工具ID集合，用于避免重复处理
     * @return 返回处理后的数据内容，用于即时显示或进一步处理
     */
    private String handleJsonMessageChunk(String chunk, StringBuilder chatHistoryStringBuilder, Set<String> seenToolIds) {
        // 解析 JSON 数据块为 StreamMessage 对象
        ...
        switch (typeEnum) {
            // 处理 AI 响应消息类型
            case AI_RESPONSE -> {
                // 将 JSON 数据解析为 AiResponseMessage 对象
